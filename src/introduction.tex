...
considered in the context of \kl{well-quasi-orderings}. The three alternatives
we consider are the \kl{prefix relation}, the \kl{suffix relation}, and the
\kl{infix relation}. Unfortunately, it is an easy check that all of these
...
relation}. In \cref{infixes:sec}, we prove that any countable quasi-ordering
with finite initial segments can be embedded into the \kl{infix relation} of a
language (\cref{count-embed-infix:thm}), we obtain the analogue of
\cref{prefixes:thm} for languages recognized by finite automata
(\cref{infix-finite-automata:thm}). Finally, in \cref{amalgamation:sec}, we
