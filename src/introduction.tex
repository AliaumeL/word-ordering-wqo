...
on their \kl{ordinal invariants}. In particular, we highlight the central role of
\kl{bounded languages} as the one that are well-behaved with respect to being
\kl{well-quasi-ordering} by the \kl{infix relation} \cref{infixes-bounded:sec}.\todo[inline]{
 i tried to rewrite it but im not sure i understand this sentence :)
}
