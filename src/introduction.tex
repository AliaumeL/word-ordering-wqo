...
\intro{order type} of a \kl{well-quasi-ordered} set is the smallest ordinal
that cannot be embedded into it. For the \kl{prefix relation} and the \kl{infix
relation}, we conclude that the order type of a \kl{well-quasi-ordered} set is
at most $k \times \omega$, and its width is finite.\todo{check this + cite}.
...
\kl{subword relation} can be embedded into the \kl{infix relation}. The former
being a \kl{well-quasi-ordered} with \kl{order type} $\omega^\omega$
\todo{cite}. However, not all is lost: we show that with some mild restrictions
we can recover much of the simple structure of the \kl[prefix relation]{prefix}
and \kl{suffix relation}:
...
systems} \cite{ASZZ24}, which abstracts the notion of systems whose runs can be
