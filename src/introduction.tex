...
as a core combinatorial tool powering many termination arguments, and were
...
words and finite trees labeled by elements of a well-quasi-ordered set~\cite{HIG52,KRU72}.
...
finite words over a \kl{well-quasi-ordered} alphabet $(X, \preceq)$ is \kl{well-quasi-ordered}
...
$wu = v$), and the \intro{infix relation} ($u \intro*\infleq v$ if there exist
...
Unfortunately, it is easy to see that none of these relations yield
well-quasi-ordered sets as soon as the alphabet contains two distinct letters:
for instance, the infinite sequence of words $(a b^n a)_{n \in \Nat}$ is \kl{well-quasi-ordered} by
...
Indeed, there have been recent breakthroughs in deciding whether a given order
...
related to the ``complexity'' of the underlying quasi-order $(X, \leq)$~\cite{SCHMITZ17}.
One way to measure this complexity is to consider its so-called \kl{ordinal
invariants}: for instance, the \kl{maximal order type} (or \kl{m.o.t.}, $\oType{X}$),
...
\kl{ordinal height} ($\oHeight{X}$)\cite{schmidt81}, and the \kl{ordinal
width} ($\oWidth{X}$) \cite{kriz90b}. Therefore, when characterizing
...
that Lemma 5.1 from \cite{DBLP:journals/ita/Kuske06} implies that there are
...
\kl{downward closed} languages, that is, languages closed under taking
...
\cref{infixes-summary:fig}. We furthermore notice that for \kl{downward
...
\begin{figure}[h]
  ...
  \begin{tabular}{c|p{6cm}|c|c}
      ...
      \addlinespace
      \kl{downward closed}  & \cref{dw-closed-wqo-charac:thm}: finite union of infixes of \kl{ultimately uniformly recurrent words} & $< \omegaOrd^2$ & $< \omegaOrd^3$ \\
      \bottomrule
    \end{tabular}
...
models respectively tailored to \kl{downward closed} languages and to
\kl{bounded languages}. For \kl{downward closed} languages, we consider a
...
so-called \emph{derivation relations} were already analysed by Bucher,
...
\kl{downward closed} languages, characterize them using a notion of
