...
consider its so-called \kl{ordinal invariants}: for instance, the
\kl{maximal order type} (or m.o.t.), originally defined by De Jongh and
...
other parameters\todo{cite sylvain width/ HDR?}: the \kl{ordinal height}
introduced by Schmidt \cite{schmidt81}, and the \kl{ordinal width} of Kříž
...
\begin{enumerate}
    \setcounter{enumi}{5}
    \item There is a decision procedure for checking whether a language $L$ 
        recognized by an \kl{amalgamation system} is \kl{well-quasi-ordered}
        by the \kl{prefix relation}, the \kl{suffix relation},
        or the \kl{infix relation}. Furthermore, if it is \kl{well-quasi-ordered}, 
