...
the set of finite \kl{infixes} of a finite, infinite,
or bi-infinte word $w \in \Sigma^* \cup
...
\begin{lemma}
  \label{pure-morphic-uur:lem}
  Let $w \in \Sigma^{\Nat}$ be a \kl{pure morphic sequence}
  generated by a morphism $f \colon \Sigma^* \to \Sigma^*$ and a letter $a \in
  \Sigma$ such that $f(n)$ starts with $a$.
  Assume furthermore that $f$ is \kl(morphic){growing}, i.e., that 
  $f(a) = a s$ where $s \in \Sigma^+$ is a non-empty word
  such that $|f^n(s)| \to \infty$ when $n \to \infty$.
  Then, the following are equivalent:
  \begin{itemize}
    \item $w$ is \kl{ultimately uniformly recurrent},
    \item \begin{enumerate}
          \item 
            for every letters $b,c \in \Sigma$,
            if $b$ is \kl(morphic){growing} (i.e., $|f^n(b)| \to \infty$ when $n \to \infty$)
            and both $b$ and $c$ appear infinitely often in $w$, 
            then $c$ appears in $f^m(b)$ for some $m \in \Nat$,
          \item
            there are only finitely many 
            \kl(worphic){bounded words}
            (i.e., words $u \in \Sigma^*$ such that $|f^n(u)|$ is bounded when $n \to \infty$)
            that appear as infixes of $w$.
    \end{enumerate}
  \end{itemize}
\end{lemma}
\begin{proof}
  Assume that $w$ is \kl{ultimately uniformly recurrent}.
  We have $N_0$ such that $w_{\geq N_0}$ is \kl{uniformly recurrent}.
  Let $b,c \in \Sigma$ be two letters such that $b$ is \kl(morphic){growing}
  and $c$ appears infinitely often in $w$.
  Then, $c$ appears in every large enough infix of $w_{\geq N_0}$.
  Because $b$ appears infinitely often in $w$, 
  $f^n(b)$ is an infix of $w_{\geq N_0}$ for all $n$'s,
  and in particular $c$ appears in $f^n(c)$ for some $n$ large enough so that 
  $|f^n(b)|$ is larger than the bound of \kl{uniform recurrence} of $w_{\geq N_0}$.

  Let us now prove the second item. Let $u$ be a \kl(worphic){bounded word}
  that appears as an infix of $w$, by construction, it cannot contain
  any \kl(morphic){growing} letter, otherwise $|f^n(u)|$ would not be bounded.
  Because $w_{\geq N_0}$ is \kl{uniformly recurrent}, there exists a bound $M_0$ 
  on the distance between two consecutive \kl(morphic){growing} letters in 
  $w_{\geq N_0}$. Now, because $f$ is \kl(morphic){growing},
  there exists a \kl(morphic){growing} letter $b \in \Sigma$ that 
  appears infinitely often in $w$. As a consequence, the 
  \kl(morphic){bounded words} that appear as infixes of $w$ have length 
  at most $N_0 + M_0$.

  Conversely, assume that the two items are satisfied. Let us prove that $w$ is
  \kl{ultimately uniformly recurrent}. First, there exists $N_0$ such that
  every letter that appears in $w_{\geq N_0}$ appears infinitely often in
  $w_{\geq N_0}$ because $\Sigma$ is finite.

  \todo[inline]{finish the proof}
  following~\cite[Theorem 4]{NIPR09} we argue that $w_{\geq N_0}$ is
...
\end{lemma}
\begin{proof}
  Let us first assume that the morphism $f$ that generates $w$ is
  \kl(morphic){growing}. If it is not the case, then $w$ is 
  ultimately periodic, and a fortiori \kl{ultimately uniformly recurrent}.
  It is possible to decide whether $f$ is \kl(morphic){growing}
  by standard techniques on morphic words.
  ...
  On the other hand, one can leverage the characterization of
  \cref{pure-morphic-uur:lem}. Indeed, it is shown in \cite[Proposition
  5]{NIPR09} that if the second item of \cref{pure-morphic-uur:lem} is not
  satisfied, then there exists a finite witness of that fact that can be
  computed from the morphism $f$. Similarly, to witness that the first item of
  \cref{pure-morphic-uur:lem} is not satisfied, one can guess two letters $a,b
  \in \Sigma$ and check that $a$ is \kl(morphic){growing}, both $a$ and $b$
  appear infinitely often in $w$, and that $b$ does not appear in $f^n(a)$ for
  any $n \in \Nat$. The first two conditions can be checked effectively, and
  the last one can be checked by noticing that if $b$ appears in $f^n(a)$ for
  some $n \in \Nat$, then it also appears in $f^m(a)$ for some $m \leq
  |\Sigma|$.
