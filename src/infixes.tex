...
combinatorial argument on regular langugaes stated in
...
\subsection{Amalgameation Systems}
...
\subsection{Downwards Closed Subsets}
\label{infixes-downwards:subsec}

In the light of \cref{reg-wqo-iff-dwclosed-wqo:cor} one may
wonder whether the \kl{downwards closed} languages for the \kl{infix relation}
satisfy a similar characterization in terms of union of \kl{chains} and
\kl{grids} as stated in
\cref{infix-finite-automata:thm,bounded-language:thm,infix-amalgamation:thm}.
Let us first remark that from our previous results we obtain the following
lemma that illustrates how all the previous characterizations of
\kl{well-quasi-ordered} languages for the \kl{infix relation} are collapsing in
the case of \kl{downwards closed} languages.
...
patterns, hence \kl{regular} \textbf{TODO cite}.
However, this is not the case for the \kl{infix relation}, as we will now
illustrate with the following two examples.
...
\begin{example}
    ...
    of excluded infixes of the form $ab^ka$ and $ba^kb$ where $k \geq 1$.
\end{example}

Let $w$ be the Thue-Morse word \textbf{CITE}, and $L$ be the set of infixes of
$w$. By construction $L$ is an \emph{infinite} \kl{downwards closed} for the
\kl{infix relation}. Let us argue that $L$ is \kl{well-quasi-ordered} for the
\kl{infix relation}, but is not \kl{regular}. First, $L$ cannot be \kl{bounded}
because $w$ is \emph{cube free}, that is, no infix of $w$ can be written $uuu$
for some word $u$ \textbf{CITE}. Furthermore, $w$ is a \intro{uniformly
recurrent word} \textbf{CITE}, that is, for every infix $u$ of $w$, there
exists $k \geq 1$ such that $u$ occurs in every infix $v$ of $w$ of size at
least $k$. This implies that $L$ is \kl{well-quasi-ordered} for the \kl{infix
relation}. We conclude thanks to \cref{dwclosed-infixes-wqo:lem},
that $L$ is not \kl{regular}.
