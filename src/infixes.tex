...
\begin{proofof}{infix-amalgamation:thm}
    ...
    $\GapWord{f_2}{i}$. 
    This means that $\canrun(r_i)$ has the shape
    \begin{equation*}
        v_0^{x_1} u_0 v_0^{x_2} u_0 v_0^{x_3} \canrun(r_i)_1 
        v_1^{x_4} u_1 v_1^{x_5} u_1 v_1^{x_6} \canrun(r_i)_2 
        \cdots
        v_{\ell}^{x_k} u_{\ell}
        v_{\ell}^{x_{k+1}}  u_{\ell}
        v_{\ell}^{x_{k+2}}
        \canrun(r_i)_{\ell}
        u v^i u
        \canrun(r_i)_{\ell+1}
    ...
    Now, leveraging \cref{pumping-periods:lem}, we get that $u v^k u$ is an
    infix of some $x^p$ where $x$ has size smaller or equal than $u$ and $v$.
    This proves  that $u$ and $v$ are comparable for the prefix, infix, and
    suffix relations. In particular, we have proven that $\GapLanguage{r}{i}$
    is bounded, because of \cite[Lemma 4.1]{ASZZ24}, when $0 < i <
    \card{\canrun(r)}$.

    For the edge cases, we do a very similar construction. For $i = 0$, we let
    $\canrun(r_i) = v^k u \canrun(r_1) \dots$, we conclude similarly. For $i =
    \card{\canrun(r)}$, we let $\canrun(r_i) = \dots
    \canrun(r)_{\card{\canrun(r) - 1}} u v^k$.
\end{proofof}
