...
\begin{proof}
    ...
    Consider a sequence $\seqof{u_i}$ of words in $L$. Without loss of
...
This motivated the following theorem, which states that for \kl{downwards
...
\AP A subset $I \subseteq X$ is \intro{directed} if, for every $x,y \in I$,
there exists $z \in I$ such that $x \leq z$ and $y \leq z$. Given a
...
\kl{downwards closed} and \kl{directed} for the \kl{order relation} $\leq$. In
our case, a \kl{well-quasi-ordered} \kl{order ideal} for the \kl{infix
relation} is the set of finite \kl{infixes} of a finite or bi-infinte word $w
\in \Sigma^\Nat \cup \Sigma^\Rel $ (\cref{bi-infinite:lem}). Therefore, we will
...
\begin{proof}
    ...
    as $\seqof{w_i}$. Now, let us build the sequence $\seqof{u_i}$ by induction
    as follows: $u_0 = w_0$, and $u_{i+1}$ is a word that contains $u_i$ and
    $w_i$, which exists in $L$ because $L$ is \kl{directed}. Since $L$ is
...
\begin{proof}
    ...
    of words $\seqof{w_i}$ that are finite \kl{infixes} of $w$. Because $w$ is
    ...
    $\seqof{w_i}$ can be found starting at the position $i$ in $w$. Indeed, if
    ...
    therefore assume that all words in the sequence $\seqof{w_i}$ are such that
...
Before proving \cref{small-ordinal-invariants:thm}, let us restrict 
our attention to a simpler case: 

The \cref{small-ordinal-invariants:thm} is a direct consequence of the
following remarks. 
