...
\begin{lemma}{\cite[Lemma 5.1]{DBLP:journals/ita/Kuske06}}
    ...
    \begin{enumerate}
        ...
        \item 
            ...
            (that is, $(X, \preceq)$ has \intro{finite initial segments}).
    \end{enumerate}
\end{lemma}
...
\begin{theorem}
    ...
    \kl{infix relation} if and only if it is included in a finite union of 
...
\begin{remark}
    ...
    \defined \setof{ \prod_{i = 0}^n w_i }{ n \in \Nat }$. By definition, $L$ is a
...
It is the core result that powers the analysis done in
...
\begin{lemma}
    ...
    belongs to $L$
    for vectors $\vec{n} \in \Nat^k$ with all coordinates arbitrarily large.
    Then, 
    there exist $x,y \in \Sigma^+$ of size 
    at most $\max \setof{\card{v_i}}{1 \leq i \leq k}$
    such that
    for all
    $\vec{n} \in \Nat^{k}$
    one of the following holds:
    $w[\vec{n}] \in \InfPeriodChain{x} u_i \InfPeriodChain{y}$
...
\begin{proofof}{bounded-language:thm}
    ...
    (\cref{inf-period-chain:lem}).\qedhere
\end{proofof}
...
\begin{corollary}
    ...
    that is \kl{well-quasi-ordered} by the \kl{infix relation}.
    Then, the \kl{ordinal width} of $L$ is less than $\omega^2$,
    its \kl{ordinal height} is at most $\omega$,
    and its \kl{maximal order type} is less than $\omega^3$.
    Furthermore, those three bounds are tight.
\end{corollary}
\begin{proof}
  Upper bounds are a direct consequence of \cref{bounded-language:thm}.
  For the tightness, remark that the \kl{ordinal width}, \kl{ordinal height}
  and \kl{maximal order type} of the language $a*b*$ are respectively 
  $\omega$, $\omega$ and $\omega^2$. Thus, by using finite unions of languages
  of the form $a_i*b_i*$, one can reach the desired bounds
  $\omega \cdot k$, $\omega$ and
  $\omega^2 \cdot k$.\footnote{Note that one can encode these languages using a binary alphabet.}\qedhere
\end{proof}
...
\cref{bounded-wqo-dwclosed:cor} is reminiscent of a similar result for the
...
\begin{example}
    ...
    \kl(language){bounded}, is \kl{downward
...
on this sequence and its properties \cite{ALSHA99}. We are using 
the notations $\Sigma^\Nat$ for the set of infinite words over the alphabet $\Sigma$,
and will be using $\Sigma^\Rel$ for the set of bi-infinite words over $\Sigma$.
...
\begin{proof}
    ...
    of $u_k$, hence the sequence $\seqof{u_i}[i \in \Nat]$ is \kl(sequence){good}.\qedhere
\end{proof}
...
\begin{proof}
    ...
    w_k^*$. Since $\LMorse$ is infinite and \kl{downward closed}, there exists a
    word $u \in \LMorse$ such that $u = w_i^3$ for some $1 \leq i \leq k$. This is a contradiction,
    because $u \infleq \ThueMorse$, which is \kl{cube-free}.\qedhere
\end{proof}
...
\end{lemma}
\begin{proof}
	We first show that $\omegaOrd$ is an upper bound for each of these measures,
    ...
    if and only if they are equal. Hence, there cannot be a finite bound
    ...
    type}, we conclude that the \kl{maximal order type} of $\LMorse$ is also $\omegaOrd$.\qedhere
\end{proof}

\AP We prove in the upcoming \cref{dw-closed-wqo-charac:thm} that the status of
the \kl{Thue-Morse sequence} is actually representative of \kl{downward
...
\AP To connect infixes of a (bi)-infinite word to \kl{downward closed}
...
\kl{downward closed} and \kl(subset){directed} for the relation $\leq$. In our
case, a \kl{well-quasi-ordered} \kl{order ideal} for the \kl{infix relation} is
the set of finite \kl{infixes} of a finite, infinite, or bi-infinite word $w \in
...
\begin{proofof}{dw-closed-wqo-charac:thm}
  ...
  \kl{downward closed} for the \kl{infix relation}. Because it is a
  ...
  \kl{ultimately uniformly recurrent}.\qedhere
\end{proofof}
...
\ifappendix 
naturally follow
(the proof is in \cref{small-ordinal-invariants:thm:proof} page
...
\begin{lemma}
    ...
    Let $w \in \Sigma^\Rel$ be an \kl{ultimately uniformly recurrent} bi-infinite word. 
    Then, the \kl{ordinal width}
...
\kl{downward closed} language are relatively small.

\begin{corollary}
  \label{small-ordinal-invariants:thm}
  \proofref{small-ordinal-invariants:thm}
    Let $L$ be a \kl{well-quasi-ordered} \kl{downward closed} language for the \kl{infix relation}.
...
particular,
\ifappendix
it follows that one can decide if an \kl{automatic
sequence} is \kl{ultimately uniformly recurrent}, 
a proof of this folklore
result can be found in the appendix at \cref{automatic-uur:lem}. 
\else
it follows that one can decide if an \kl{automatic
sequence} is \kl{ultimately uniformly recurrent}.
\fi
