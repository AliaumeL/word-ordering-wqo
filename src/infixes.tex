...
\begin{proof}
    ...
    $\LMorse$. 
    The bound of the \kl{ordinal height} holds for any language $L$,
    since a descending chain for $(\Sigma^*, \infleq)$ is isomorphic to one for $(\Nat, \leq)$ \todo{please check this is a good argument}.
    For the \kl{maximal order type}, we remark that
    the \kl{uniform recurrence} of $\ThueMorse$ means that
    the maximal length of a \kl{bad sequence} is determined by its first element,
    hence that it is at most $\omegaOrd$.
    Finally, because the \kl{ordinal width} is at most the \kl{maximal order type} \todo{add reference},
...
the set of finite \kl{infixes} of a finite \todo{$\Sigma^\Nat$ is an infinite word}
or bi-infinte word $w \in
