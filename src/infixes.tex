...
\todo[inline]{Rewrite this paragraph to cite precise results.}
It is the case that \kl{bounded languages} have at most a polynomial factor
complexity, and that the \kl{Thue-Morse sequence} has a linear factor
complexity. More generally, as we will see, most ``concrete'' examples of
infinite words have a low factor complexity: \kl{automatic sequences} have at
most a linear factor complexity, and \kl(morphic){morphic sequences} 
have a polynomial factor complexity. In
that sense, the \cref{exponential-factor-complexity:lem} shows that one can
still construct ``pathological'' examples of infinite words that are
\kl{well-quasi-ordered} for the \kl{infix relation}, but have a high factor
complexity.

Note that because of \cite[Theorem 24]{NIPR09}, any \kl{uniformly recurrent}
\kl{morphic sequence} has a linear factor complexity. In particular, we
conjecture that any \kl{ultimately uniformly recurrent} \kl{morphic sequence}
has a linear factor complexity too. 
...
One can go beyond \cref{automatic-wqo:thm} and consider
\kl(morphic){morphic sequences}, which are obtained by iterating a
\kl(substitution){substitution} on a finite alphabet, and then applying a
\kl(homomorphism){homomorphism}. The \kl{Fibonacci sequence} is an example of
a \kl(morphic){morphic sequence}. It is known that one can decide whether a
\kl(morphic){morphic sequence} is \kl{uniformly recurrent}
\cite[Theorem 1]{DURAND13}.

\todo[inline]{Define properly morphic words and 
  the questions associated to them. Also, there is a difference between
  \kl{ultimately uniformly recurrent} and \kl{uniformly recurrent}
  that is not handled here.}
\begin{theorem}
    \label{morphic-wqo:thm}
