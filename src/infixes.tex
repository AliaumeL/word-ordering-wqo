...
\begin{proof}
  ...
  \kl{maximal order type} respectively equal to $\omega \cdot k$, $\omega$ and $\omega^2 \cdot k$.
\end{proof}
...
\subsection{Downwards Closures and Bounded Languages}
...
\begin{theorem}
  \label{bounded-wqo-dw-closed-regular:thm}
  Let $L$ be a \kl{downwards closed} and \kl{well-quasi-ordered} language
  of $\Sigma^*$ for the \kl{infix relation}. The following are equivalent:
  \begin{enumerate}
    \item $L$ is a \kl{bounded language},
    \item $L$ is a \kl{regular language},
    \item There exists a finite set $E \subset_{\mathsf{fin}} (\Sigma^*)^3$ such that
      $L = \bigcup_{(x,u,y) \in E} \InfPeriodChain{x} u \InfPeriodChain{y}$.
  \end{enumerate}
\end{theorem}

\subsection{General Downwards Closed Languages}
...
\todo[inline]{aliaume: write it}
Let us conclude this section with a few remarks on the notion of
\kl{factor complexity} of languages. 

\begin{lemma}
  There exists a language $L$ with exponential factor complexity 
