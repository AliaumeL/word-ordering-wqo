...
\begin{definition}
    \textbf{TODO}. An \intro{amalgamation system}
    is a triple such that.
\end{definition}

Let us recall some examples of \kl{amalgamation systems} given by the authors
of the original paper: regular languages \cite[Theorem 5.3]{ASZZ24}, VASS as a
consequence of \cite[Theorem 5.5]{ASZZ24}, context-free languages as a
consequence of \cite[Theorem 5.10]{ASZZ24}.

\begin{lemma}
    Let $L$ be a language recognized by an \kl{amalgamation system}
    and assume that $L$ is \kl{well-quasi-ordered}
    by the \kl{infix relation}. Then,
    for every minimal run $\rho$,
    the \kl{gap languages} of $\rho$
    are \kl{subsemigroup languages}.
\end{lemma}
