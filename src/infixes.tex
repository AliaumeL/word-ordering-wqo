...
\begin{proof}
  ...
  On the other hand, we can also provide an upper bound on the possible values
  of $N_0$ if $w$ is \kl{ultimately uniformly recurrent}. Let us write $f(a) =
  a s$ for some finite word $s \in \Sigma^*$. If $s$ is $f$-bounded, i.e., if
  the size of $f^n(s)$ is bounded by some constant $K \in \Nat$, then $w$ is
  ultimately periodic, and the prefix $N_0$ that may not be periodic can easily
  be bounded by looking at the morphism $f$. Let us consider the case when $s$
  is not $f$-bounded. If the word $w$ is \kl{ultimately uniformly recurrent},
  then there exists a bound $N_0$ such that $w_{\geq N_0}$ is \kl{uniformly
  recurrent}. In particular, for a large enough $n$, there exists $k_0$  such
  that for all $k \geq k_0$, the word $f^n(s)$ is an \kl{infix} of $f^{n +
  k}(s)$.
  \todo[inline]{not clear how to conclude ...}
