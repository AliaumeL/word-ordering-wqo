...
We are now ready to restate and prove our main theorem.
...
\kl{subword embeddings} as morphisms. Given a \kl{subword embedding} $f \colon
u \to v$ between two words $u$ and $v$, there exists a unique decomposition $v
= \GapWord{f}{0} u_1 \GapWord{f}{1} \cdots \GapWord{f}{k-1} u_k \GapWord{f}{k}$
where $\GapWord{f}{i} = v[f(i)+1:f(i+1)-1]$ for all $1 \leq i \leq k-1$,
$\GapWord{f}{k} = v[f(k)+1:\card{v}]$, and $\GapWord{f}{0}   = v[1: f(1)-1]$.
We say that $\intro*\GapWord{f}{i}$ is the $i$-th \intro{gap word} of $f$.
...
\begin{definition}
    ...
    and $\canrun \colon R \to (\Sigma \cup \set{\varepsilon})^*$ is a 
    function computing a \emph{canonical decompostion} of a run.
    Given a run $r \in R$, and $i \in \range[0]{\card{\canrun(r)}}$, we define
    the \intro{gap language} of $r$ at position $i$ as $\GapLanguage{r}{i} \defined
...
runs, and the proper ``gluing" of runs together to obtain new runs.
