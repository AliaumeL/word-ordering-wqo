...
\kl{downward closed} by the \kl{infix relation} as \kl{infixes} of infinite
...
class of all regular languages, or the class of all context-free languages. Such
...
between two words $u \in \Sigma^*$ and $v \in \Sigma^*$ is an order-preserving function $\rho
...
\AP Given a \kl{subword embedding} $f \colon u \to v$ between two words $u = u_1 \cdots u_k$ and
...
\begin{definition}
	...
	The \emph{yield} of a run is obtained by projecting away the separator symbol 
    \cansep~from the canonical decomposition, i.e. 
    $\intro*\yieldrun(\rho) = \project_\Sigma( \canrun[\rho])$. 
    The language recognized by an \kl{amalgamation system} is $\yieldrun(R)$.
...
\begin{lemma}
  ...
  \proofref{gap-words-prefix-ordered:lem}
	Let $L$ by an \kl{amalgamation language} recognized by $(\Sigma, R, E, \canrun)$
    that is well-quasi-ordered by $\infleq$. Let $\rho$ be a run with
    $\canrun[\rho] = a_1 \cdots a_n$, and let $\sigma, \tau$
    be runs with $\rho \runleq_f \sigma$ and $\rho \runleq_g \tau$. 
...
\begin{lemma}
    ...
    Let $L \subseteq \Sigma^*$ be a \kl{downward closed} language for the
...
Under these assumptions, one can improve on \cref{bounded-language:thm} into an effective
