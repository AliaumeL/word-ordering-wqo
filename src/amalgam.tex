...
\begin{theorem}[label=infix-amalgamation:thm,restate=infix-amalgamation:thm]
    ...
    If $L$ is well-quasi-ordered by the infix relation then $L$ is
    \kl(language){bounded}.
\end{theorem}

\todo[inline]{There are missing effective restrictions in the statement of the theorem: we should
have descriptions of the languages, (effective) stability under transductions etc.}
...
\AP For the next result, we impose a mild restriction by requiring the language
class to be closed under \kl{rational transductions}. This property holds for
most common language classes, including all known \kl{amalgamation systems}. A
\intro{rational transduction} is a function $f \colon \Sigma^* \to \Gamma^*$
that can be defined by a finite automaton with output, or equivalently by a
finite set of rules of the form $u \to v$ where $u \in \Sigma^*$ and $v \in
\Gamma^*$. We write $f(L) = \setof{f(w)}{w \in L}$ for the image of a language
$L$ by a rational transduction $f$. A class of languages $\mathcal{C}$ is
\intro{closed under rational transductions} when, for every language $L \in
\mathcal{C}$, and every rational transduction $f \colon \Sigma^* \to \Gamma^*$,
the language $f(L)$ is in $\mathcal{C}$.\todo{check these definitions ... Do we allow
the alphabet to vary?}

\AP A class $\mathcal{C}$ of languages is an \intro{effective amalgamative
class} whenever there exists a finite presentation of the languages \dots\todo{
define formally}. And for every $L \in \mathcal{C}$, there exists an
...
Note that this implies boundedness and simultaneous boundedness of languages in
$\mathcal{C}$ is decidable \cite{ASZZ24}. \todo{define formally}
...
\begin{proofof}{infix-wqo-is-emptiness:thm}
    ...
    just consider the transduction \todo{fresh symbols? can we assume we have those??}
