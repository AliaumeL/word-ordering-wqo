...
called \kl{effective amalgamative classes} (\kcref{effective amalgamative
classes}). In the following theorem, we prove that under such assumptions,
testing \kl{well-quasi-ordering} is inter-reducible to testing whether a
...
\AP We say that a class $\mathcal{C}$ of languages is an \kl{effective
...
class of languages $\mathcal{C}$ is \intro{effectively closed under rational
transductions} when, for every language $L \in \mathcal{C}$, and every rational
...
\cite{BERST79} for a primer on \kl{rational transductions}.
...
\begin{proofof}{infix-wqo-is-emptiness:thm}
    ...
    \mapsto L \cap \Sigma^* \setminus R$  is a \kl{rational transduction}
    because $\Sigma^* \setminus R$ is regular. Since $\mathcal{C}$ is
    \kl{closed under rational transductions}, we can therefore reduce the
    ...
    then $L$ cannot be \kl{well-quasi-ordered} by the \kl{infix relation}
    ...
    Then, for any language $L \in \mathcal C$, $f(L)$ is \kl{well-quasi-ordered}
    by \kl{prefix} if and only if $L$ is finite if and only if $L$ is finite.
    Hence, we can decide if $L$ is finite, and therefore if $L$ is empty 
    by enumeration.
    \todo{I don't understand the notation $\Sigma^* \times \set{a,b}^*$, so I did 
    a worse but correct proof for me.}
\end{proofof}
