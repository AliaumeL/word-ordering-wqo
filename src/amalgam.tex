...
\todo[inline]{%
  We have studied infinite words, but there are other ways to represent
  languages, such as finite automata, context-free grammars, or vector
  addition systems with states. In this section, we show that even for
  a generalisation of these systems, we can decide whether the language is
  well-quasi-ordered by the infix relation, and that in turns,
  they all fit in the bounded languages studied in the previous section.
}
